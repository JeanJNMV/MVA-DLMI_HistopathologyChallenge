...
Deep learning models for computational pathology often suffer from performance degradation when deployed across medical centers due to domain shifts induced by variations in staining protocols, imaging devices, and tissue preparation. In this work, we address the problem of out-of-distribution (OOD) generalization for histopathology patch classification in the context of a multi-center setting. We propose a robust pipeline built upon DINOv2 self-supervised vision transformers, combining partial fine-tuning of the backbone with stain-aware data augmentation and test-time augmentation (TTA). 

We systematically investigate the impact of model capacity, fine-tuning depth, and domain generalization techniques such as MixStyle and stain-space augmentations. Through extensive ablation studies and hyperparameter optimization, we show that selectively unfreezing a small number of transformer blocks significantly improves generalization to unseen centers. While advanced augmentation strategies provide marginal gains, we find that a carefully tuned fine-tuning strategy coupled with TTA yields the best performance. 

Our final model achieves strong accuracy on unseen domains, demonstrating that leveraging pretrained representations with controlled adaptation is an effective approach to mitigate domain shift in histopathology. These results highlight the importance of balancing invariance and adaptability when designing models for real-world clinical deployment. The full code is available at: \url{https://github.com/JeanJNMV/MVA-DLMI_HistopathologyChallenge}.
